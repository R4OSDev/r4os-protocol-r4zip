// From Zig 0.16.0, MIT; see LICENSE-Zig and THIRD_PARTY_NOTICES.md.
// R4ZIP change: stored blocks honor the remaining per-call output budget.
    ...
    sw: switch (d.state) {
        ...
        .stored_block => |remaining_len| {
            const out: []u8 = if (remaining != 0)
                ...
                &.{};
            var limited_out: [1][]u8 = .{std.Io.Limit.limited(@min(remaining, remaining_len)).slice(out)};
